...
This is the intended v0.1 architecture. Phase 3A adds deterministic Leader planning inputs and a dossier gate without invoking a model. Phase 3B adds a local, citation-backed engineering knowledge layer that can optionally supply reference chunks to the Leader prompt. Phase 4A adds the default interactive shell, guided first-run setup, local provider discovery, versioned non-secret personal preferences, and deterministic settings precedence. Phase 4B adds a real bounded, read-only Codex CLI Leader adapter and a strict structured plan-draft conversation. Phase 4C adds provider-owned live model catalogs, a Textual full-screen Fleet application, an explicit approve → exact durable run → start handoff, and an in-memory live output monitor; a catalog is live-or-unavailable with no fallback list, and a draft is still not execution authorization. Phase 2 local Worker execution, Phase 1 contracts, approval guards, SQLite, and JSONL remain intact. Verification, review, and application remain design targets; execution stops at `VERIFYING`.
...
| `shell.py` | Implemented (Phase 4A/4B): prompt-based interactive shell, guided first-run setup, and the in-memory planning conversation (`/new`, `/plan`, `/cancel`, Free text). Prompts are separated from decisions and IO is injectable. The shell never starts a Worker or creates a run or approval. |
| `settings_cli.py` | Implemented (Phase 4A): the `settings` editor and read-only `settings show` surface. |
| `leader_adapters.py` | Implemented (Phase 4B): a separate Leader capability and bounded read-only adapters. Codex CLI is implemented via `codex exec --sandbox read-only --output-schema --output-last-message`; Claude Code and OpenCode have no Leader adapter. |
| `planning.py` | Implemented (Phase 4B): in-memory conversation state, single bounded prompt compilation, one Leader invocation per explicit turn, strict response parsing, and plan-draft validation against the user's explicit selections. No persistence and no execution. |
| `catalog_contracts.py` | Implemented (Phase 4C): `ModelCatalog`, `DiscoveredModel`, `ModelCatalogStatus`, `ModelCatalogSource`, and `ProviderCapabilities`. A verified catalog must carry models; there is no fallback catalog type. |
| `catalog.py` | Implemented (Phase 4C): provider-owned live catalog adapters. Codex uses bounded JSON-RPC over `codex app-server --stdio` (`initialize`/`initialized`, paginated `model/list`); OpenCode parses `opencode models --verbose`; Claude honestly reports unavailability. No network call outside the provider's own CLI/app-server. |
| `monitor.py` | Implemented (Phase 4C): thread-safe, bounded, in-memory `FleetMonitor` and view models plus `worker_layout`/`page_count`. Raw output lives only here and is never persisted. |
| `handoff.py` | Implemented (Phase 4C): explicit approve → durable run → `PLAN_EXECUTION` approval → start handoff reusing Phase 1/2 guards. |
| `fleet.py` | Implemented (Phase 4C): headless controller coordinating catalogs, planning, preferences, approval, and live execution. Fully testable without a terminal. |
| `tui.py` | Implemented (Phase 4C): Textual full-screen setup, planning, and fleet screens that render controller/monitor view models. Only imported when a real terminal is present. |

The adapter interface receives an explicit provider, model, role, task context, worktree path, and limits. It translates these into argv arrays and normalizes exit status and bounded output metadata. `doctor` checks configured executables locally with `--version`/`--help`; a specific model's remote availability cannot be established offline. A CLI rejection fails visibly and never triggers a substitute provider or model. Codex CLI and Claude Code are the initial Worker adapters; OpenCode is catalog-only for now.
...
The bare `patchfleet` command is the default experience. When both stdin and stdout are terminals, it ensures personal preferences exist—running the guided first-run setup if not—then opens a lightweight prompt-based shell. In a non-interactive terminal it prints a concise explanation plus help and exits with a non-zero usage status; it never calls `input()` or blocks. All earlier subcommands (`doctor`, `plan`, `run`, `charter`, `context`, `leader`, `architecture`, `knowledge`) are unchanged, and `settings` is added. The shell supports `/new [request]`, `/plan`, `/cancel`, `/settings`, `/doctor`, `/help`, and `/quit`.

The shell uses no full-screen TUI dependency. Presentation lives in `shell.py`; provider discovery, preference persistence, precedence resolution, and planning live in separate testable modules. Prompts are separated from decisions so the first-run flow can be driven with injected input and output. The shell displays the effective Leader, default Workers, maximum parallel Worker count, and discovered provider availability. Free text starts a planning conversation (Phase 4B) or answers the Leader's latest question; the shell never starts a Worker, creates a run, provisions a worktree, records an approval, or executes verification.
...
Provider discovery is local and read-only. It resolves each provider's configured executable (a built-in name, a project override, or a personal override) and runs only bounded `--version`/`--help` checks. It never contacts a provider service, invokes an agent task, or downloads anything. It reports Worker-execution capability, Leader-planning capability, and detection-only support. Codex CLI has both a Worker execution adapter and a read-only Leader planning adapter; Claude Code has a Worker execution adapter only; OpenCode is detection-only. A provider that does not expose the required explicit model/flags is installed but not capable for that role and cannot be selected; PatchFleet never substitutes a provider or model.
...
## Leader conversation boundary (Phase 4B)

The interactive shell runs a real Leader conversation for Codex CLI. Each explicit user turn compiles one bounded prompt and invokes exactly one Leader subprocess: `codex exec --model <selected> --sandbox read-only --ephemeral --output-schema <file> --output-last-message <file> -`, run from the target repository root with the prompt on stdin. It uses an argv array, the strongest documented read-only sandbox, explicit model selection, wall-clock timeout, bounded stdout/stderr, bounded response size, and cancellation support. It never retries, falls back, or makes an extra paid call, and it never modifies Git or starts a Worker.

The response contract is versioned and strict: exactly `questions` or `plan_draft`. A `plan_draft` embeds an ordinary PatchFleet `Plan` and is accepted only after existing `Plan` validation and checks that the Leader matches the user's selection and every Worker/Reviewer assignment matches a user-selected Worker provider/model. Malformed, oversized, incomplete, invalid, or substituted output is rejected with a clear error and no automatic correction. A repository HEAD change between context capture and response invalidates the turn.

The prompt contains the request and answer history, repository root and pinned HEAD, exact Leader/Worker selections, maximum parallelism, a bounded structural repository summary from the Phase 3A context machinery, and the exact response JSON Schema. Charter and RAG context are optional and are not required for ordinary usage. Conversation history, compiled prompts, and raw Leader output are session-local and in-memory: they are never written to personal preferences, execution JSONL events, the execution SQLite store, or source control, and raw prompts/outputs are never logged as execution events.

A validated draft is readiness for review, not execution authorization. Phase 4B never calls `run create`, `run approve`, `run start`, or a Worker adapter. Planning requires a target Git repository; outside one, only non-planning commands run.

## Live catalogs, Fleet TUI, and handoff (Phase 4C)

The interactive entrypoint launches a Textual application when a real terminal and Textual are available, and otherwise falls back to the Phase 4B prompt shell. All headless commands remain unchanged. The application is thin: `tui.py` renders `fleet.FleetController` state and `monitor.FleetMonitor` view models, and sends actions back to the controller; it never touches persistence or provider processes directly.

Model selection is provider-owned and live. `catalog.py` asks exactly one installed, already-authenticated provider CLI what models it can reach. Codex starts `codex app-server --stdio`, performs the documented JSON-RPC `initialize`/`initialized` handshake, calls `model/list` with an opaque cursor until `nextCursor` is exhausted, and shuts the process group down with bounded time, output, malformed-JSON handling, and cancellation. OpenCode parses the stable `provider/model` + JSON blocks of `opencode models --verbose`. Claude Code reports `CATALOG_UNAVAILABLE` because its CLI documents aliases and explicit ids but no account-specific catalog command. There is no curated fallback list; a model is selectable only from a fresh successful catalog, and no paid call occurs merely to validate a model. Catalogs are cached in memory for the active session only, and refresh is explicit. A saved selection missing from a fresh catalog is shown unavailable and blocks new execution until reselected. Preferences store only the selected id plus catalog source and timestamp, never raw catalog data or credentials.

Approval preserves every existing guard. A validated draft is inspected, then the user explicitly approves: this creates the durable run via `create_run` and records the existing `PLAN_EXECUTION` approval for the exact fingerprint and revision through `approve_run`. Only a separate "Start fleet" confirmation calls `start_run`, which provisions worktrees and schedules Workers through the unchanged Phase 2 foundation. A plan merely existing never starts anything.

`processes.supervise` gained an optional display-only `on_output` callback, and `execution.start_run`/`_execute_task` gained an optional `ExecutionObserver` plus optional whole-run and per-task cancellation events. These are advisory: they never change guard semantics or the safe event payload allowlist. Raw Worker output is passed only to the in-memory monitor, where it is sanitized, bounded, and labeled when truncated; it is never written to events, SQLite, JSONL, preferences, or telemetry. The Leader panel is a coordination/status view driven by scheduler and process events, not a model review. Execution still ends at `VERIFYING`; verification, review, application, commit, and push are later phases.

Terminal behavior: the Leader panel occupies roughly the upper 35%; Worker panels use one full-width panel, two columns, a 2×2 grid, or a paged 2×2 grid by count, and switch to a single active panel with keyboard paging on small terminals. `q` never silently abandons a running fleet: the UI asks whether to cancel (and quit) or keep the fleet running in the session, and there is no background daemon.
