...
A validated draft is readiness for review, not execution authorization. Phase 4B never calls `run create`, `run approve`, `run start`, or a Worker adapter. Phase 4C will connect a reviewed, user-approved draft to the existing exact-plan approval flow. Planning requires a target Git repository; outside one, only non-planning commands run.
